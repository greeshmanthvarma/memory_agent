...
import type { CreateMemoryResponse } from "@/types"

interface LogMemoryDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
}

export default function LogMemoryDialog({ open, onOpenChange }: LogMemoryDialogProps) {
  ...
  async function handleSaveMemory() {
    ...
    try {
      const response = await fetch('/api/memory/create', {
        ...
        body: JSON.stringify({ content: memory.trim(), memory_type: 'explicit', tags: [] }),
        credentials: 'include',
      })
      if (response.ok) {
        const data: CreateMemoryResponse = await response.json()
        if (data.is_duplicate) {
          toast.info(data.duplicate_type === 'exact' ? 'Memory already exists (exact match found)' : 'Similar memory already exists')
        } else {
        ...
        toast.error((errorData as { detail?: string }).detail || 'Failed to save memory')
      }
    } catch {
