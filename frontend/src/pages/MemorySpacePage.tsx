...
export default function MemorySpacePage() {
  ...
  async function fetchMemories() {
    ...
    setIsLoading(true)
    try {
      const response = await fetch('/api/memory', { credentials: 'include', headers: { 'Content-Type': 'application/json' } })
      if (response.ok) {
        const data: Memory[] = await response.json()
        setMemories(data || [])
      } else {
        const errorData = await response.json().catch(() => ({}))
        toast.error((errorData as { detail?: string }).detail || 'Failed to fetch memories')
      }
    } catch (error) {
      toast.error('Failed to fetch memories: ' + (error instanceof Error ? error.message : 'Unknown error'))
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <div className="h-full overflow-y-auto p-6">
      ...
      <MemoryDialog open={isDialogOpen} onOpenChange={setIsDialogOpen} memory={selectedMemory} fetchMemories={fetchMemories} />
    </div>
  )
}
